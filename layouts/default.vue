<template>
  <header class="fixed top-0 inset-x-0 text-pink-500 bg-purple-500 shadow-md z-50">
    <div class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
      ...
      <nav class="hidden lg:flex gap-8 items-center">
        <NuxtLink v-for="link in links" :key="link.name" :to="link.to"
          class="text-lg hover:text-gray-50 duration-200 relative" :class="{ 'active-link': isActive(link.to) }">
          {{ link.name }}
          <span v-if="isActive(link.to)"
            class="absolute -top-2 left-1/2 transform -translate-x-1/2 w-full h-0.5 bg-pink-500"></span>
        </NuxtLink>
        <a href="https://open.spotify.com/playlist/1zb6DNXLiK9UL7VqimfjMA" target="_blank" rel="noopener"
    ...
    <div v-if="isMenuOpen" class="lg:hidden px-6 pb-4 flex flex-col gap-4 bg-purple-500">
      <NuxtLink v-for="link in links" :key="link.name" :to="link.to" @click="isMenuOpen = false"
        class="text-lg hover:text-gray-50 duration-200">
        {{ link.name }}
      </NuxtLink>
      <a href="https://open.spotify.com/playlist/1zb6DNXLiK9UL7VqimfjMA" target="_blank" rel="noopener"
  ...
  <footer class="pt-24 pb-12 text-purple-500 bg-pink-500">
    <section class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div class="flex items-center gap-2 text-xl text-purple-500">
        ...
        <a href="https://www.instagram.com/ersatz.festival" target="_blank" rel="noopener noreferrer"
