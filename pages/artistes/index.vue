<template>
    <div class="mx-auto max-w-5xl px-6 pt-8 pb-12">
        ...
        <div class="mt-10 grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
            <NuxtLink
                v-for="entry in allArtists"
                :key="entry.artist.slug"
                :to="`/artistes/${entry.artist.slug}`"
                class="group block"
            >
                ...
                <p class="mt-2 text-lg sm:text-xl uppercase leading-tight text-[var(--color-primary)] group-hover:text-[var(--color-primary-hover)]">
